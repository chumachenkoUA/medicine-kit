...
export function MedicineBentoGrid({ medicines }: MedicineBentoGridProps) {
  ...
  const getProgressTone = (item: MedicineDashboardItem) => {
    if (isLowStock(item)) return "bg-amber-500 dark:bg-amber-400"
    if (isExpiringSoon(item)) return "bg-orange-500 dark:bg-orange-400"
    return "bg-primary"
  }
      ...
      <BentoGrid className="grid-cols-1 auto-rows-[16rem] md:grid-cols-2 xl:grid-cols-3">
        {medicines.map((medicine, index) => {
          ...
          return (
            ...
            >
              <div className="pointer-events-none absolute -right-10 -top-10 size-24 rounded-full bg-primary/15 opacity-0 blur-2xl transition-opacity duration-300 group-hover:opacity-100 dark:bg-primary/25" />
